...
TIM_HandleTypeDef htim4; // Para PWM en PB8/PB9
...
void Init_PWM(void)
{
    ...
    // Configurar PB8 (TIM4 CH3) y PB9 (TIM4 CH4)
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    ...
    // Configurar TIM4 para 10 kHz con resolución de 12 bits
    // Timer clock = 84 MHz (APB1 = 42 MHz * 2)
    // Periodo = (84,000,000 / 10,000) - 1 = 8399
    htim4.Instance = TIM4;
...
int main(void)
{
    ...
    // Crear controlador de motor usando PB8 (TIM4 CH3) y PB9 (TIM4 CH4)
    DRV8833_Motor motor(&htim4, TIM_CHANNEL_3,
                        GPIO_PIN_8, GPIOB,
                        GPIO_PIN_9, GPIOB);
    motor.begin();
    ...
    char init_msg[] = "DRV8833 Controller - PB8/PB9 PWM 10kHz 12bit\r\n";
    ...
    while (1)
    {
        // Convertir ADC (0-4095) a velocidad (-100 a 100)
        int8_t speed = (int8_t)((adc_value / 4095.0f) * 200 - 100);
